"""Generador de datos sinteticos para ConectaMax (Spec 02).

Crea >= 2.000 clientes coherentes con el contrato de config/settings.py y los
expande al modelo relacional (Spec 01). La variable 'abandono' surge de varios
drivers ponderados + ruido (nunca una regla rigida, Plan v3.2 §29.3).
...
# --- Dominios reales (del CSV de Roberto) ---
CIUDADES = [
    # ciudad, id_sucursal, region, peso
    ("Santiago",     1, "Metropolitana",      0.30),
    ("Valparaiso",   3, "Valparaiso",         0.10),
    ("Concepcion",   4, "Biobio",             0.10),
    ("Antofagasta",  2, "Antofagasta",        0.08),
    ("Puerto Montt", 5, "Los Lagos",          0.07),
    ("La Serena",    3, "Coquimbo",           0.07),
    ("Temuco",       4, "La Araucania",       0.07),
    ("Rancagua",     1, "O'Higgins",          0.06),
    ("Iquique",      2, "Tarapaca",           0.06),
    ("Valdivia",     5, "Los Rios",           0.05),
    ("Arica",        2, "Arica y Parinacota", 0.04),
]
SUCURSALES = [
...
# servicio: id, codigo, nombre, tipo, precio
SERVICIOS = [
...
NOMBRES = ["Ana", "Luis", "Marcela", "Diego", "Camila", "Jorge", "Paula", "Felipe",
           ...
           "Pablo", "Sofia", "Hector", "Natalia", "Cristian", "Monica", "Francisco"]
APELLIDOS = ["Rojas", "Paredes", "Soto", "Salinas", "Fuentes", "Molina", "Herrera",
             "Vargas", "Castro", "Nunez", "Medina", "Campos", "Vega", "Torres",
             "Leon", "Contreras", "Aguilera", "Bravo", "Carrasco", "Silva"]

# Pesos del modelo de churn (Spec 02 §3)
...
def construir(n: int):
    ...
    cpesos = np.array([c[3] for c in CIUDADES]); cpesos = cpesos / cpesos.sum()
    ...
    for i in range(1, n + 1):
        ...
        for k, sv in enumerate(servicios_cli):
            ...
            contratos.append((cid, IDSRV[sv], tipo_contrato if k == 0 else "Mensual",
                              fecha_alta, None, "activo", monto, 1 if k == 0 else 0))

        # --- facturas (6 periodos) + pagos ---
        ...
        for m in range(6):
            ...
            periodo = fmes.strftime("%Y-%m")
            emision = fmes.isoformat()
            venc = (fmes + timedelta(days=20)).isoformat()
            atrasado = m in idx_atrasados
            facturas.append((cid, periodo, monto_total, emision, venc,
                             "vencida" if atrasado else "pagada"))
            # marcador para enlazar pago tras conocer id_factura (se inserta luego)
            pagos.append((cid, periodo, monto_total, atrasado))

        # --- reclamos (en los ultimos 6 meses) ---
        ...
        # --- interacciones (correlacionadas con reclamos) ---
        n_int = int(np.clip(reclamos_n + rng.poisson(0.5), 0, 10))
        canales_i = ["soporte", "llamada", "app", "sucursal"]
        for _ in range(n_int):
            dias = int(rng.integers(0, 180))
            interacciones.append((cid, (HOY - timedelta(days=dias)).isoformat(),
                                  rng.choice(canales_i), rng.choice(["consulta", "soporte", "reclamo", "venta"]),
...
def poblar(db_path: str, n: int, exportar_csv: bool):
    ...
    for t in ["predicciones", "interacciones", "reclamos", "pagos", "facturas",
              "contratos", "clientes", "servicios", "sucursales"]:
    ...
    # facturas + pagos (necesitamos id_factura): insertar factura, luego su pago
    cur = con.cursor()
    for (cid, periodo, monto, emision, venc, estadof), (pc, pp, pmonto, atrasado) in zip(fac, pag):
        ...
        if atrasado:
            ...
            cur.execute("INSERT INTO pagos (id_factura,fecha_pago,monto_pagado,estado,dias_atraso)"
                        " VALUES (?,?,?,?,?)", (idf, venc, monto, "atrasado", dias_at))
        else:
            cur.execute("INSERT INTO pagos (id_factura,fecha_pago,monto_pagado,estado,dias_atraso)"
    ...
    if exportar_csv:
        ...
        rows = con.execute(f"SELECT {','.join(cols)} FROM comportamiento_cliente ORDER BY id_cliente").fetchall()
